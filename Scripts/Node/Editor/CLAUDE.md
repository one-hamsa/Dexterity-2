...
| `GraphNodeEditor.cs` | `GraphNode` | Inspector: aggregated-state banner, list of sources on the host with per-source 3-state override pill, live-preview toggle. |
...
- **target**: replaced with a dropdown of `GraphNode` + aggregators on the SAME GameObject as the source. ObjectField is hidden — designers can't accidentally point at a foreign GO.
